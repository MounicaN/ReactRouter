...
loadXMLDoc(a){            
  ...
  render() {
    return (
        <div>
        <div className="left">
        <button className = "but" onClick={this.TopStories} >TopStories</button><br/>
        <button className = "but" onClick={this.MostRecentStories} >Most Recent Stories</button><br/>
        <button className = "but" onClick={this.Health} >Health</button><br/>
        </div>
        <div className = "right">
        {
            this.state.item.map((item,i) =>
            <div key = {i}>
               <a href = {item.link} style={{fontSize: '20px'}}>{item.title}</a><br/>
                <p style={{fontSize: '20px'}}>{item.description}</p> 
                <b style={{color: 'red',fontSize: '20px'}}>{item.pubDate}</b>
                <hr/>
            </div>
        )
        }
